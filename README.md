...
|---|---|---:|---|
| [Collision](notebooks/other-ideas/full_stokes_collision.ipynb) | Opposing side inflow for 30 yr; compensating stress-free surface outflow | $6.00\times10^{-2}$ yr$^{-1}$ | 0 overturned segments; no bracketed Pleistocene minimum |
| [Traveling basal patch](notebooks/other-ideas/full_stokes_basal_patch.ipynb) | 8 km low-drag Robin patch; 600 yr in a frame moving at 40 m yr$^{-1}$ | $2.98\times10^{-3}$ yr$^{-1}$ | 0 overturned segments; no bracketed Pleistocene minimum |
| [Migrating Ice Wall](notebooks/other-ideas/full_stokes_ice_wall.ipynb) | 200 m surface step; 1 km of relative migration over 100 kyr | $6.62\times10^{-6}$ yr$^{-1}$ in the wall zone | 0 overturned segments; no bracketed Pleistocene minimum |
| [Waddington positive control](notebooks/full_stokes_waddington_fold.ipynb) | A 25 m-amplitude, 500 m-wavelength seed wrinkle; 25 kyr of n=3 gravity-driven shear | $1.08\times10^{-3}$ yr$^{-1}$ | 4544 overturned segments; 10 bracketed Pleistocene minima |
...
Issue [#16](https://github.com/bradlipovsky/allan-hills-stratigraphy/issues/16) updates the three core notebooks to Glen $n=3$ flow. Each gravity-driven slab uses a stated strain-rate regularization and a softness calibrated to retain the earlier surface speed, so differences isolate the nonlinear depth profile rather than an arbitrary speed change. The Waddington positive control leaves initially flat layers unfolded but rotates a seeded 17° limb through vertical after 3.2 kyr. By 25 kyr, the best synthetic core contains a 5.20–0.63–5.20 Ma top-to-bottom triplet.

The [ALHIC1901 inverse-fit notebook](notebooks/alhic1901_full_stokes_fit.ipynb) takes the next step: it fits a compact, initially ordered basal wrinkle to the uploaded Shackleton et al. (2025) age data using velocity from a 160 m-thick n=3 full-Stokes slab. The target is the observed three-fold age–depth structure, not merely an arbitrary reversal. Its three modeled old-age peaks lie within 0.29 m of the data-defined peaks, with 0.370 Ma pointwise RMS age misfit; doubling both mesh dimensions changes sample predictions by at most 0.019 Ma. This is a qualified positive result: it requires a 10.3 m-amplitude, 27.4 m-wavelength seed with an 81.1° limb, so the visual match supplies a target for transient models rather than a unique reconstruction.

The [alternating surface-mass-balance notebook](notebooks/alternating_smb_seed.ipynb) tests whether neighboring accumulation and ablation bands can create that seed. It solves a periodic two-dimensional n=3 full-Stokes unit cell and evaluates 2592 layer outcomes. No ordered case reaches both 8 m amplitude and 80° dip. The best present-rate case at the fitted wavelength reaches 6.0 m and 87.8°; later n=3 slab shear could overturn it quickly, but its amplitude remains too small. At 149 m depth the 27.4 m surface response is only $1.1\times10^{-15}$ of its surface value, so any smooth surface-made seed must form near the surface before entering the deeper shear zone.

The [unified surface-forcing and ALHIC1901 notebook](notebooks/unified_smb_alhic1901_fit.ipynb) replaces the freely fitted wrinkle with a material-coordinate map generated by the n=3 surface cell, then shortens and shears that packet with the n=3 slab. The best tested history uses an aggressive 0.10 m yr$^{-1}$, 60 m-wavelength surface pattern for 200 yr, horizontal shortening to 0.87 of its original wavelength, and 47.7 kyr of later shear. It reproduces all three target folds with 0.412 Ma pointwise RMS misfit and peak-depth errors no larger than 0.31 m; a factor-two slab refinement changes the predicted curve by at most 0.008 Ma. The positive result specifically requires a surface accumulation/scour boundary, not merely a smooth surviving wrinkle, and should be treated as a target history rather than a unique Little Ice Age reconstruction.

The matched [linear-rheology comparison](notebooks/unified_smb_alhic1901_fit_linear.ipynb) repeats that complete workflow with Glen $n=1$ in both Stokes stages. It also reproduces the three folds, with 0.420 Ma RMS misfit and peak-depth errors no larger than 0.33 m, but selects a 27.42 m surface pattern, shortening to 0.77, and 34.5 kyr of later shear. Thus the present age data do not discriminate strongly between $n=1$ and $n=3$ in this idealized construction, although the preferred forcing and duration change. Both fits require the surface accumulation/scour boundary.

The [coupled-initialization notebook](notebooks/coupled_initialization_histories.ipynb) clarifies that the existing surface stage already contains uniform horizontal drift; it omits only depth-dependent slab shear during seed formation. Seven impulse-matched $n=3$ histories compare that baseline with flow throughout, a 10 kyr flow-first delay, smooth and joint ramps, and a weak 2 kyr pulse. Turning slab flow on during or before the 200--400 yr seed changes little: the refitted flow-throughout and flow-first cases both retain 0.412--0.413 Ma RMS misfit and the three target folds. Surface-stage shear strain is at most 0.0015, compared with about 13.7 after basal transfer. The weak 2 kyr pulse changes the fixed prediction by as much as 1.29 Ma, but refitting only phase and shear duration still gives 0.411 Ma RMS. The joint-ramp result changes by at most 0.004 Ma when the time step is halved and 0.006 Ma when both Full-Stokes meshes are coarsened by a factor of two. In this idealization, short-event onset order is therefore not identifiable; the prescribed surface-to-bed transfer remains the more consequential assumption.
